...
const char *SQLChecks::chkSQL(Line *line, ScriptImage *img)
{
	const char *cp;

	if(getMember(line))
		return "command does not use member options";

	if(!useKeywords(line, "=results"))
		return "invalid or unknown keyword used";
